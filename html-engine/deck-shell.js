...
(() => {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Icons: decks author <i data-lucide="name"> placeholders, but the
      // shell historically never loaded lucide — icons rendered blank
      // unless the deck bundled its own loader. Auto-load the vendored
      // copy (no CDN: deterministic for live/PDF/PPTX export) and swap
      // placeholders to inline SVG. Idempotent if the deck loaded lucide
      // itself.
      this._initLucide();
      this._render();
      this._loadNotes();
    ...
    _snapshotAuthorCss() {
      ...
      let authorCss = Array.from(document.styleSheets).map((sh) => {
        try {
      ...
      // Kill entry animations inside the thumbnail shadow root. Design
      // systems use opacity-from-0 rise-in animations on slide children;
      // those animations don't run in cloned thumb roots so the elements
      // stay frozen at the FROM state (opacity 0 / translateY) and the
      // thumb looks empty or mis-positioned. Forcing forwards fill +
      // zero duration snaps the rendered state to the END frame.
      authorCss += `
        *, *::before, *::after {
    ...
    _initLucide() {
      ...
      const run = () => {
        ...
        catch (e) { /* malformed icon name shouldn't kill the deck */ }
      };
      if (window.lucide) return run();
      let s = document.querySelector('script[data-deck-lucide]');
      if (s) { s.addEventListener('load', run); return; }
      s = document.createElement('script');
      s.src = '/html-engine/lucide.min.js';
      s.setAttribute('data-deck-lucide', '');
      s.onload = run;
    ...
    _toggleSkip(i) {
      ...
      try { if (window.parent !== window) window.parent.postMessage({ type: 'workpod-slide', index: this._index, total: this._slides.length }, '*'); } catch (e) {}
    }
  ...
  // ─── Inline-edit handshake with the workpod parent frame ─────
  // When the deck is loaded into the workpod artifact preview iframe,
  // the parent can toggle edit mode and request a save. We add
  // contentEditable to text-only leaf elements (so the user clicks a
  // headline and types, but the layout containers stay fixed) and
  // suppress deck-shell's own keyboard navigation while editing so
  // arrow keys move the text cursor instead of the slide.
  let editMode = false;
  // Tags we never make editable — media, SVG primitives, scripts, etc.
  // Plus structural slide elements (section is the slide container).
  const SKIP_TAGS = new Set([
    'SECTION', 'IMG', 'SVG', 'VIDEO', 'AUDIO', 'IFRAME', 'CANVAS', 'PICTURE',
    'SOURCE', 'TRACK', 'OBJECT', 'EMBED', 'SCRIPT', 'STYLE', 'LINK', 'META',
    'NOSCRIPT', 'TEMPLATE', 'HEAD', 'HTML', 'BODY', 'DECK-STAGE',
    'PATH', 'RECT', 'CIRCLE', 'LINE', 'POLYGON', 'POLYLINE', 'G', 'DEFS',
    'MASK', 'USE', 'TITLE', 'DESC', 'CLIPPATH', 'PATTERN', 'TEXT', 'TSPAN',
    'BR', 'HR', 'INPUT', 'BUTTON', 'SELECT', 'TEXTAREA', 'FORM', 'LABEL',
    'UL', 'OL',   // Lists themselves — let <li> handle each item
    'TABLE', 'THEAD', 'TBODY', 'TFOOT', 'TR',   // Cell-level only
  ]);
  // An element is "text-bearing" if it directly contains text (or only
  // inline children that contain text). Structural containers — divs
  // with block-level children — are skipped so the user edits the
  // leaf text, not the layout.
  const isTextBearing = (el) => {
    if (SKIP_TAGS.has(el.tagName)) return false;
    if (!el.textContent.trim()) return false;
    // Reject if any child is itself text-bearing — we want the
    // INNERMOST text container, not its parents.
    for (const c of el.children) {
      if (SKIP_TAGS.has(c.tagName)) continue;
      const d = getComputedStyle(c).display;
      // Treat block-ish display as a real child to walk past.
      if (d === 'block' || d.startsWith('flex') || d.startsWith('grid')
          || d === 'table' || d === 'list-item' || d === 'grid' || d === 'inline-block') {
        // Only reject if THIS child has meaningful text — empty
        // block siblings (decorative divs) shouldn't disqualify the
        // parent.
  ...
  const enableEditMode = () => {
    ...
    editMode = true;
    dirtyNotified = false;
    document.body.setAttribute('data-workpod-editing', '1');
    document.querySelectorAll('deck-stage section').forEach((sec) => {
      sec.querySelectorAll('*').forEach((el) => {
        if (!isTextBearing(el)) return;
        el.setAttribute('contenteditable', 'true');
        el.dataset.workpodEditable = '1';
      });
    });
    // First user keystroke or paste flips the dirty flag → parent
    // shows the prominent Save / Discard buttons.
    document.addEventListener('input', onAnyEdit, true);
    // Add a faint dashed outline + soft glow on hover so the user knows
    // what's clickable. Injected once.
    if (!document.getElementById('workpod-edit-style')) {
      const style = document.createElement('style');
      style.id = 'workpod-edit-style';
      style.textContent = `
        [data-workpod-editing] [data-workpod-editable] {
          outline: 1px dashed rgba(0, 113, 227, 0.30);
          outline-offset: 3px;
          cursor: text;
          border-radius: 3px;
        }
        [data-workpod-editing] [data-workpod-editable]:hover {
          outline-color: rgba(0, 113, 227, 0.55);
          background: rgba(0, 113, 227, 0.05);
        }
        [data-workpod-editing] [data-workpod-editable]:focus {
  ...
  const disableEditMode = () => {
    ...
    document.body.removeAttribute('data-workpod-editing');
    document.querySelectorAll('[data-workpod-editable]').forEach((el) => {
      el.removeAttribute('contenteditable');
      delete el.dataset.workpodEditable;
    });
  };
  // Suppress deck-shell keyboard nav while editing — arrows should
  // move the text caret, not the slide.
  window.addEventListener('keydown', (e) => {
    if (!editMode) return;
    const navKeys = new Set(['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown',
                              'PageUp', 'PageDown', 'Home', 'End', ' ']);
    if (navKeys.has(e.key)) e.stopImmediatePropagation();
  }, true);

  // "t" toggles the slide thumbnail rail (works in normal mode or
  // present mode). Avoids the existing "r" binding which resets the
  // deck to slide 0. Bypassed during edit so typing "t" inside a
  // heading doesn't toggle.
  window.addEventListener('keydown', (e) => {
    if (editMode) return;
    ...
    if (!stage) return;
    const nextOn = !stage._railVisible;
    window.postMessage({ type: '__deck_rail_visible', on: nextOn }, '*');
  });

  // Fullscreen → present mode. When the iframe (or any descendant)
  // enters fullscreen, set _presenting on the deck-stage so the rail
  // and overlay hide automatically. Mirror exit so leaving fullscreen
  // restores the rail.
  document.addEventListener('fullscreenchange', () => {
    const stage = document.querySelector('deck-stage');
    if (!stage) return;
    const presenting = !!document.fullscreenElement;
    window.postMessage({ __omelette_presenting: presenting }, '*');
  });
  // Receive parent messages.
  window.addEventListener('message', (e) => {
    if (!e.data || typeof e.data !== 'object') return;
    // Host-driven navigation — the workpod comments panel jumps the
    // preview to the slide a comment refers to.
    if (e.data.type === 'workpod-goto' && typeof e.data.index === 'number') {
      const stage = document.querySelector('deck-stage');
      if (stage && typeof stage._go === 'function') stage._go(e.data.index, 'api');
    }
    ...
    if (e.data.type === 'workpod-edit-save') {
      // Clean the edit markers + contentEditable before serializing so
      // the saved HTML doesn't carry the editing scaffolding.
      const wasEditing = editMode;
      disableEditMode();
      const styleNode = document.getElementById('workpod-edit-style');
      if (styleNode) styleNode.remove();
      const html = '<!doctype html>\n' + document.documentElement.outerHTML;
      // Re-enable editing if it was on — the user may want to keep going.
